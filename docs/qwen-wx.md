...
同步接口（推荐）
HTTP调用
通义万相文生图模型支持同步接口，一次请求即可获得结果，调用流程简单，推荐用于多数场景。
...
wan2.6-t2i：默认值为 1280*1280。总像素在 [768*768, 1440*1440] 之间且宽高比范围为 [1:4, 4:1]。例如，768*2700符合要求。
...
生成图片的数量。取值范围为1~4张，默认为4。测试阶段建议设置为1，方便低成本验证。
